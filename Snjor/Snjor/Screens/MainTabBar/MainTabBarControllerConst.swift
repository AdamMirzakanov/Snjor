//
//  MainTabBarControllerConst.swift
//  Snjor
//
//  Created by Адам Мирзаканов on 23.10.2024.
//

import Foundation

enum MainTabBarControllerConst {
  static let hideTabBarAnimationDuration: TimeInterval = 0.8
  static let showTabBarAnimationDuration: TimeInterval = 0.6
  static let gradientViewShowDuration: TimeInterval = 1.0
  static let searchButtonTag: Int = 1
  static let settingsButtonTag: Int = 2
  static let pressingScale: CGFloat = 0.6
  static let duration: CGFloat = 0.1
  static let defaultOpacity: CGFloat = 0.4
  static let maxOpacity: CGFloat = 1.0
  static let photoListButtonTag: Int = 0
  static let gradientViewHeight: CGFloat = 250.0
  static let gradientViewYPosition: CGFloat = 150.0
  static let firstColorLocation: CGFloat = 0.05
  static let secondColorLocation: CGFloat = 0.7
  static let buttonSize: CGFloat = 23.0
  static let customBarVerticalInset: CGFloat = 95.0
  static let customBarHorizontalInset: CGFloat = 90.0
  static let customBarWidthOffset: CGFloat = customBarHorizontalInset * 2
  static let customBarHeight: CGFloat = 50.0
  static let photosButtonIcon: String = "photos"
  static let searchButtonIcon: String = "search"
  static let settingsButtonIcon: String = "slider"
}
